...
const fetchHistory = async () => {
  try {
    ...
    history.value = res.data.map(item => {
      if (locale.value === 'ZH' && item.userIntent) {
        item.userIntent = item.userIntent
            .replace('Build a premium pet social platform', '构建高级宠物社交平台')
            .replace('Personal Blog', '个人博客')
            .replace('jewelry brand website', '珠宝品牌网站')
      }
      return item
    })
  } catch (err) {
    ...
    <script>
      ...
      document.addEventListener('click', (e) => {
        ...
        if (component) {
          ...
          badge.innerHTML = '<span style="opacity:0.6;margin-right:4px">W×H</span> ' + Math.round(rect.width) + ' × ' + Math.round(rect.height)
  ...
  <div class="h-full flex flex-col bg-black text-gray-300 select-none overflow-hidden">
    <nav class="h-16 flex items-center justify-between px-8 border-b border-white/5 bg-black/40 backdrop-blur-3xl z-40 shrink-0">
      ...
      <div class="flex items-center gap-4">
        <button class="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/5 hover:border-white/20 transition-all group"
        ...
        <button class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 hover:border-white/20 transition-all"
                @click="toggleLang">
    ...
    <div class="flex-1 flex overflow-hidden relative">
      <main :class="[isWorkbenchMode ? (isSidebarOpen ? 'w-[75%]' : 'w-full') : 'w-full flex items-center justify-center']"
        ...
        <div v-if="!isWorkbenchMode && !loading" class="max-w-2xl w-full px-6 text-center">
          ...
          <div class="relative group max-w-xl mx-auto">
            ...
            <button :disabled="!prompt.trim() || loading" class="absolute bottom-4 right-4 bg-blue-600 text-white px-8 py-3 rounded-2xl font-black flex items-center gap-2"
        ...
        <div v-if="isWorkbenchMode" class="h-12 bg-black/40 border-b border-white/5 flex items-center justify-center gap-4 shrink-0 px-4">
          <div class="flex items-center bg-white/5 rounded-lg p-1 border border-white/5">
            <button
                ...
                :key="d.id" :class="[deviceType === d.id ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300']"
          ...
          <button :class="[isDebugMode ? 'bg-purple-600 text-white shadow-lg border-purple-500' : 'text-gray-500 border-white/10 hover:border-white/20']"
        ...
        <div v-if="isWorkbenchMode" class="flex-1 relative overflow-hidden flex justify-center bg-[#0d0d0d] no-scrollbar">
          <transition name="fade">
            <div v-if="activeTab === 'design'" :key="'design'"
              ...
              <div :class="[deviceType === 'desktop' ? 'p-0' : 'p-12']"
                ...
                <div
                  ...
                  <iframe :srcdoc="injectInspectScript(result?.prototypeHtml)"
            ...
            <div v-else-if="activeTab === 'preview'" :key="'preview'" class="h-full w-full">
              <Sandpack :files="sandpackFiles" :options="{ editorHeight: '100%', externalResources: ['https://cdn.tailwindcss.com'] }" class="h-full" template="vue3"
                        theme="dark"/>
            </div>
          </transition>
        </div>
      ...
      <aside v-if="isWorkbenchMode && isSidebarOpen"
        ...
        <div class="flex-1 p-6 overflow-auto space-y-6">
          ...
          <button v-if="result?.status === 'DONE'" class="w-full py-4 bg-white text-black font-black rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl"
                  @click="handleConfirm">
            立即编码
          </button>
        </div>
      ...
      <transition name="drawer">
        <div v-if="isHistoryOpen"
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
            <div v-for="proj in history" :key="proj.id" class="group p-4 bg-white/5 border border-white/5 rounded-2xl cursor-pointer hover:bg-white/10 hover:border-blue-500/30 transition-all relative overflow-hidden"
